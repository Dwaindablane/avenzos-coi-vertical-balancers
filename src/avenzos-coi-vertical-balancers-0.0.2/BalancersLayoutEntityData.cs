...
internal class BalancersLayoutEntityData : IModData
{
    ...
    private void AddZippers(ProtoRegistrator registrator, StaticEntityProto.ID id, string name, IoPortShapeProto portShape, EntityCostsTpl costs, int height, string prefabPath, Option<string> iconPath)
    {
        ...
        var zipper = new ZipperProto(id, strings, layout, costs.MapToEntityCosts(registrator), requiredPower, false, new LayoutEntityProto.Gfx(prefabPath, default(RelTile3f), iconPath, ColorRgba.White, hideBlockedPortsIcon: true, null, categories, useInstancedRendering: false));
